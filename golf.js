...
import { trackSpline, percent } from './track.js';

var golf, golfMesh, golfMaterial;

export function addGolf() {
  ...
  golf.faceVertexUvs[0][2] = [
    new THREE.Vector2(0, 1),
    new THREE.Vector2(1, 0),
    new THREE.Vector2(0, 0),
  ];
  golf.faces.push(new THREE.Face3(5, 7, 6));
  golf.faceVertexUvs[0][3] = [
    new THREE.Vector2(1, 0),
    new THREE.Vector2(1, 1),
    new THREE.Vector2(0, 1),
  ];
...
export function setGolf() {
  golfMesh.position.x =
    trackSpline.points[
      percent + 10 >= trackSpline.points.length - 1 ? percent : percent + 10
    ].x;
  golfMesh.position.z =
    trackSpline.points[
      percent + 10 >= trackSpline.points.length - 1 ? percent : percent + 10
