...
function showSplit() {
  ...
  for (var i = 0; i < parseInt(value[0]); i++) {
    ...
    var newObject = new THREE.Mesh(
      geom,
      object.material.clone(new THREE.MeshBasicMaterial())
    );
...
function addProperties(object, deep) {
  ...
  for (var property in object) {
    ...
    if (
      ...
      if (type == 'object') {
        value =
          "<div onclick=\"this.nextSibling.style.display = this.nextSibling.style.display == '' ? 'none' : '';\">+</div>" +
          '<div style="display:none">' +
          (deep ? addProperties(value) : '') +
          '</div>';
      }
      ...
      switch (currentAction.action) {
        case 'move':
          $.post(
            ...
            function () {
              currentAction.object.position.set(
                clickedPoint.x,
                0.3,
                clickedPoint.z
              );
      ...
      if (lastSelected != null) {
        ...
        for (var i = 0; i < coordinates.length; i++) {
          ...
          for (var j = 0; j < lastCoordinates.length; j++) {
            ...
            if (xOK && yOK && zOK) {
              console.log(
                formatCoordinate(coordinates[i]) +
                  ' ' +
                  object.geometry.name.split(' ')[0]
              );
              console.log(
                formatCoordinate(lastCoordinates[j]) +
                  ' ' +
                  lastSelected.geometry.name.split(' ')[0]
              );
              cornersFound++;
