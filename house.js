import { imgTexture } from '../textures.js';
import { scene } from './scene.js';
import { clickObjects } from './click-objects.js';
import { finishes, addFinish } from './finish.js';

export let houses = [];
export let house = [];
export let houseMaterial;
export let houseMesh = [];
export let roof = [];
export let roofMaterial;
export let roofMesh = [];
export let floor = [];
export let floorMaterial;
export let floorMesh = [];
export let innerWalls = [];
export let innerWallsMaterial;
export let innerWallsMesh = [];
export let housesVisible = true;
export let roofsVisible = true;
export let floorsVisible = true;
...
export function addHouse(name, points, height, color, floors) {
  if (floors == null) {
    floors = [{}];
  }
  ...
  clickObjects.push(houseMesh[index]);
  addFlatRoof(name, points, height, color);
  for (var i = 0; i < floors.length; i++) {
    var floorBottom = (height / floors.length) * i;
    var floorTop = (height / floors.length) * (i + 1);
    addFloor(name, points, floorBottom, floors[i].color);
    ...
    if (walls != null) {
      ...
      innerWall.vertices.push(
        new THREE.Vector3(points[1][0], floorBottom, -points[1][1])
      ); //rechts voor onder
      innerWall.vertices.push(
        new THREE.Vector3(points[1][0], floorTop, -points[1][1])
      ); //rechts voor boven
      innerWall.vertices.push(
        new THREE.Vector3(points[2][0], floorTop, -points[2][1])
      ); //rechts achter boven
      innerWall.vertices.push(
        new THREE.Vector3(points[2][0], floorBottom, -points[2][1])
      ); //rechts achter onder
      ...
      innerWall.vertices.push(
        new THREE.Vector3(points[3][0], floorTop, -points[3][1])
      ); //links achter boven
      innerWall.vertices.push(
        new THREE.Vector3(points[3][0], floorBottom, -points[3][1])
      ); //links achter onder
      ...
      innerWall.vertices.push(
        new THREE.Vector3(points[0][0], floorTop, -points[0][1])
      ); //links voor boven
      innerWall.vertices.push(
        new THREE.Vector3(points[0][0], floorBottom, -points[0][1])
      ); //links voor onder
...
function addFlatRoof(name, points, height, color) {
  ...
  roof[index].vertices.push(
    new THREE.Vector3(points[0][0], height, -points[0][1])
  );
  roof[index].vertices.push(
    new THREE.Vector3(points[1][0], height, -points[1][1])
  );
  roof[index].vertices.push(
    new THREE.Vector3(points[2][0], height, -points[2][1])
  );
  roof[index].vertices.push(
    new THREE.Vector3(points[3][0], height, -points[3][1])
  );
  roof[index].faces.push(new THREE.Face3(0, 1, 2));
  roof[index].faces.push(new THREE.Face3(0, 2, 3));
  roof[index].computeBoundingSphere();
  roofMesh.push(new THREE.Mesh(roof[index], roofMaterial));
