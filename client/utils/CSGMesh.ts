...
class CSG {
  ...
  static fromGeometry = (geom: THREE.BufferGeometry, objectIndex?: object) => {
    let polys = [];
    let { position: posattr, normal: normalattr, uv: uvattr, color: colorattr } = geom.attributes;
    ...
    switch (polygonType) {
      ...
      case SPANNING:
        ...
        for (let i = 0; i < polygon.vertices.length; i++) {
          ...
          if ((ti | tj) == SPANNING) {
            let t =
              (this.w - this.normal.dot(vi.pos)) / this.normal.dot(tv0.copy(vj.pos).sub(vi.pos));
